...
struct IPTVPlaylistSelectionView: View {
    ...
    var filteredCategories: [IPTVPlaylist] {
        if searchText.isEmpty { return IPTVManager.categories }
        return IPTVManager.categories.filter { $0.title.localizedCaseInsensitiveContains(searchText) }
    }
            ...
            ZStack {
                ...
                ScrollView(.vertical, showsIndicators: false) {
                    VStack(alignment: .leading, spacing: 28) {
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            SectionHeader(
                                title: searchText.isEmpty ? "Catégories" : "Résultats pour \"\(searchText)\"",
